...
import {
  Truck, AlertTriangle, FileText, Boxes, ShoppingCart, BookOpen, UserCog,
  TrendingUp, Coins, HandCoins, Landmark, PiggyBank, Wallet, ArrowUpRight,
  ArrowDownRight, Flame, Download, Printer, CircleCheck, CircleMinus, Zap,
} from "lucide-react";
import { DashboardShell } from "@/components/dashboard-shell";

export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({ meta: [{ title: "Business Control · Insaf Trading" }] }),
...
const shortcuts = [
  { icon: FileText, label: "Sales Invoice", grad: "var(--gradient-primary)", to: "/module/new-invoice" as const },
  { icon: Boxes, label: "Products Catalog", grad: "var(--gradient-cool)", to: "/products" as const },
  { icon: ShoppingCart, label: "Log Purchase", grad: "var(--gradient-mint)", to: "/module/purchase-entry" as const },
  { icon: BookOpen, label: "Cash Book", grad: "var(--gradient-gold)", to: "/module/cash-book" as const },
  { icon: UserCog, label: "HR & Staff", grad: "var(--gradient-rose)", to: "/module/employees" as const },
  { icon: TrendingUp, label: "Sales Report", grad: "var(--gradient-accent)", to: "/module/sales-reports" as const },
];
...
function DashboardPage() {
  ...
  return (
    <DashboardShell
      ...
      crumb={<>Dashboard › <span className="text-foreground font-semibold">Overview</span></>}
    >
      {/* HERO */}
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-2">
            {topProducts.map((p) => {
              ...
              return (
                <div key={p.rank} className="flex items-center gap-3 p-3 rounded-xl border border-border/60 hover:border-border transition">
                  <div className="h-9 w-9 rounded-xl grid place-items-center text-primary-foreground font-bold text-sm shrink-0" style={{ background: gradByRank }}>
                    #{p.rank}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{p.name}</p>
                    <p className="text-[11px] text-muted-foreground truncate">{p.cat} · {p.qty}</p>
                  </div>
